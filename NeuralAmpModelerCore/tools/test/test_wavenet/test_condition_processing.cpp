...
std::unique_ptr<nam::wavenet::WaveNet> create_simple_wavenet(
  ...
  const int global_condition_size = 0)
{
...
void test_with_condition_dsp_and_global_condition()
{
  auto condition_dsp = create_simple_wavenet(1, 2, nullptr);
  auto wavenet = create_simple_wavenet(1, 1, std::move(condition_dsp), 1);
  ...
  NAM_SAMPLE* inputPtrs[] = {input.data()};
  NAM_SAMPLE* outputPtrs[] = {output.data()};
  const double condition = 0.25;
  wavenet->SetDefaultParams(&condition, 1);
  wavenet->Reset(48000.0, 64);
  wavenet->process(inputPtrs, outputPtrs, numFrames);
  for (const NAM_SAMPLE sample : output)
    assert(std::isfinite(sample));

  wavenet->process(inputPtrs, outputPtrs, numFrames, &condition, 1);
  for (const NAM_SAMPLE sample : output)
    assert(std::isfinite(sample));

  const double invalidCondition = std::numeric_limits<double>::quiet_NaN();
  bool rejectedNonFinite = false;
  try
  {
    wavenet->process(inputPtrs, outputPtrs, numFrames, &invalidCondition, 1);
  }
  ...
  bool rejectedInvalidDefault = false;
  try
  {
    wavenet->SetDefaultParams(&invalidCondition, 1);
  }
  catch (const std::invalid_argument&)
  {
    rejectedInvalidDefault = true;
  }
  assert(rejectedInvalidDefault);
}
