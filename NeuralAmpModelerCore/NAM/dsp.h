...
public:
  ...
  /// \brief Process audio with block-constant model parameters.
  ///
  /// Non-parametric models reject non-empty parameter vectors instead of
  /// silently ignoring them. Parametric architectures override this method.
  virtual void process(NAM_SAMPLE** input, NAM_SAMPLE** output, const int num_frames, const double* params,
                       const int num_params)
  {
    if (num_params != 0)
      throw std::invalid_argument("This model does not accept global parameters");
    process(input, output, num_frames);
  };
  /// rief Set the block-constant values used by generic operations such as
  /// prewarming. Non-parametric models accept only an empty vector.
  virtual void SetDefaultParams(const double* params, const int num_params)
  {
