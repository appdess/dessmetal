...
public:
  ...
  WaveNet(const int in_channels, const std::vector<LayerArrayParams>& layer_array_params, const float head_scale,
          ...
          const double expected_sample_rate = -1.0, const int global_condition_size = 0);
  ...
  /// \brief Process audio with a block-constant global conditioning vector.
  void process(NAM_SAMPLE** input, NAM_SAMPLE** output, const int num_frames, const double* params,
               const int num_params) override;
...
private:
  void _process(NAM_SAMPLE** input, NAM_SAMPLE** output, const int num_frames, const double* params,
                const int num_params);

  std::vector<_LayerArray> _layer_arrays;

  float _head_scale;
  const int _global_condition_size;
