...
def multi_resolution_stft_loss(
    ...
    if loss_func is None:
        raise ValueError(
            "You must provide a pre-initialized loss function for the (Mel-) MultiResolutionSTFTLoss. This is for performance reasons."
        )
    if device is not None:
        preds, targets = [z.to(device) for z in (preds, targets)]
    return loss_func(preds, targets)

# def mel_multi_resolution_stft_loss(
#     preds: _torch.Tensor,
#     targets: _torch.Tensor,
#     loss_func: _Optional[_MultiResolutionSTFTLoss] = None,
#     device: _Optional[_torch.device] = None,
# ) -> _torch.Tensor:
#     """
#     multi_resolution_stft_loss() but with mel scale.
#     B: Batch size
#     L: Sequence length
...
#     :return: ()
#     """
#     loss_func = _MultiResolutionSTFTLoss(
#         scale="mel",
#         n_bins=128,
#         sample_rate=48000 # TODO Hardcoded for now 
