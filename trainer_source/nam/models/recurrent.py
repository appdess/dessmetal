...
class LSTM(_BaseNet):
    ...
    def __init__(
        ...
        **lstm_kwargs,
        ...
        conditioning_dim = lstm_kwargs.pop("conditioning_dim")
        self._conditioning_dim = conditioning_dim
        if conditioning_dim is not None:
            input_size += conditioning_dim
        else:
            raise ValueError(
                "conditioning_dim must be set in the model config. "
            )
    ...
    def _forward(
        ...
        x: _torch.Tensor, 
        g: _torch.Tensor,
        initial_state: Optional[_LSTMHiddenCellType] = None
    ) -> _torch.Tensor:
        """
        :param x: (B,L) or (B,L,D_in)
        :param g: Conditioning vector (B, D_cond)
        :return: (B,L)
        """
        ...
        # Expand g to match the sequence length of x and concatenate
        # g shape: (B, D_cond) -> (B, 1, D_cond)
        g_unsqueezed = g.unsqueeze(1)
        # g shape: (B, 1, D_cond) -> (B, L, D_cond)
        g_expanded = g_unsqueezed.expand(-1, x.shape[1], -1)

        # Concatenate along the feature dimension
        # x shape: (B, L, D_in) and g_expanded shape: (B, L, D_cond)
        # Resulting shape: (B, L, D_in + D_cond)
        x = _torch.cat([x, g_expanded], dim=2)
