...
void dsp::ImpulseResponse::_SetWeights()
{
  ...
  {
    this->mResampled.resize(std::min(this->mRawAudio.size(), this->mMaxLength));
  ...
  else
  {
    ...
    // Only the first mMaxLength taps can ever reach the convolution kernel.
    // Bound resampling itself so an extreme source/host sample-rate ratio
    // cannot allocate or compute an unbounded intermediate vector.
    dsp::ResampleCubic<float>(
      padded, this->mRawAudioSampleRate, mSampleRate, 0.0, this->mResampled, this->mMaxLength
    );
  }
