...
public:
   ...
   * @param func The callable that processes audio at the higher sample rate. It is templated so captured lambdas do
   * not require std::function type erasure or render-time heap allocation. */
  template <typename ProcessFunc>
  void ProcessBlock(T** inputs, T** outputs, int nFrames, ProcessFunc&& func)
  {
    ...
    while (mResampler1->GetNumSamplesRequiredFor(1) == 0) // i.e. there's more to process
    {
      ...
      {
        mInvariantFailureCount.fetch_add(1, std::memory_order_relaxed);
        for (int c = 0; c < NCHANS; ++c)
          std::fill(outputs[c], outputs[c] + nFrames, static_cast<T>(0));
        return;
      }
    ...
    {
      mUnderflowCount.fetch_add(1, std::memory_order_relaxed);
      for (int c = 0; c < NCHANS; c++)
      {
        const T lastSample = populated2 > 0 ? outputs[c][populated2 - 1] : 0.0;
        for (size_t i = populated2; i < static_cast<size_t>(nFrames); i++)
        {
  ...
  unsigned int GetUnderflowCount() const { return mUnderflowCount.load(std::memory_order_relaxed); }
  unsigned int GetInvariantFailureCount() const { return mInvariantFailureCount.load(std::memory_order_relaxed); }

private:
  std::atomic<unsigned int> mUnderflowCount{0};
  std::atomic<unsigned int> mInvariantFailureCount{0};
