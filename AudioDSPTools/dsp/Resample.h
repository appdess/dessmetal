...
void ResampleCubic(const std::vector<T>& inputs, const double originalSampleRate, const double desiredSampleRate,
                   ...
                   const std::size_t maxOutputSize = std::numeric_limits<std::size_t>::max());
...
void dsp::ResampleCubic(const std::vector<T>& inputs, const double originalSampleRate, const double desiredSampleRate,
                        const double tOutputStart, std::vector<T>& outputs, const std::size_t maxOutputSize)
{
  // This is a transform, not an append operation. Clearing here keeps
  // maxOutputSize an absolute bound even when a caller reuses its buffer.
  outputs.clear();
  if (!std::isfinite(originalSampleRate) || originalSampleRate <= 0.0 || !std::isfinite(desiredSampleRate)
      || desiredSampleRate <= 0.0)
    throw std::runtime_error("Sample rates must be finite and positive");
  if (inputs.size() < 3)
    throw std::runtime_error("Cubic resampling requires at least three input samples");
  if (!std::isfinite(tOutputStart) || tOutputStart < 0.0)
  ...
  while (time < endTimeOriginal && outputs.size() < maxOutputSize)
  {
    // Find the index of the sample in the original audio file that is just
    // before the current time in the resampled audio file
    const std::size_t index = static_cast<std::size_t>(std::floor(time / timeIncrement));
