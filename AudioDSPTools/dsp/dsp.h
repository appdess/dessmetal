...
public:
  DSP();
  virtual ~DSP();
  // Reserve/initialize processing buffers away from the real-time callback.
  // Process() may subsequently resize within this capacity without allocating.
  void PrepareBuffers(const size_t numChannels, const size_t numFrames) { _PrepareBuffers(numChannels, numFrames); }
