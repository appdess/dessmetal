...
OSStatus IPlugAU::SetState(CFPropertyListRef pPropList)
{
  ...
  // RestorePreset() resets parameters before the incoming chunk is parsed. Keep
  // a complete snapshot so a malformed or unsupported state can be rejected
  // without leaving that preset reset visible to the host.
  IByteChunk previousState;
  const int previousPresetIdx = GetCurrentPresetIdx();
  if (!SerializeState(previousState))
    return kAudioUnitErr_InvalidPropertyValue;
  ...
  {
    if (UnserializeState(previousState, 0) > 0)
    {
      SetCurrentPresetIdx(previousPresetIdx);
      OnRestoreState();
    }
    return kAudioUnitErr_InvalidPropertyValue;
  }
