...
IRECT CornerButtonArea(const IRECT& rect)
{
  ...
  return mainArea.GetFromTRHC(50, 50).GetCentredInside(24, 24);
};
...
// Original vector-drawn amp stomp switch. The control remains directly bound
// to kNAMActive through IVToggleControl, so host automation, serialization,
// and the existing latency-compensated DSP bypass path are unchanged.
class NAMStompToggleControl : public IVToggleControl
{
public:
  NAMStompToggleControl(const IRECT& bounds, int paramIdx, const IVStyle& style)
  : IVToggleControl(bounds, paramIdx, "", style, "BYPASS", "ON")
  {
  }
  ...
  void Draw(IGraphics& g) override
  {
    const bool enabled = GetValue() > 0.5;
    const float pressOffset = mMouseDown ? 1.f : 0.f;
    const float disabledOpacity = IsDisabled() ? 0.42f : 1.f;
    const IColor capColor =
      (enabled ? IColor(255, 232, 48, 54) : IColor(255, 112, 30, 34)).WithOpacity(disabledOpacity);

    const float switchX = mRECT.MW();
    const float switchY = mRECT.T + 21.f + pressOffset;
    if (enabled)
      g.FillCircle(IColor(255, 255, 44, 50).WithOpacity(0.15f * disabledOpacity), switchX, switchY, 24.f, &mBlend);
    g.FillCircle(COLOR_BLACK.WithOpacity(0.58f * disabledOpacity), switchX + 1.5f, switchY + 2.f, 21.f, &mBlend);
    g.FillCircle(IColor(255, 184, 187, 192).WithOpacity(disabledOpacity), switchX, switchY, 20.f, &mBlend);
    g.FillCircle(IColor(255, 46, 48, 52).WithOpacity(disabledOpacity), switchX, switchY, 17.f, &mBlend);
    g.FillCircle(capColor, switchX, switchY, 14.5f, &mBlend);
    if (mMouseIsOver && !IsDisabled())
      g.FillCircle(COLOR_WHITE.WithOpacity(0.08f), switchX, switchY, 14.5f, &mBlend);
    g.FillCircle(COLOR_WHITE.WithOpacity(0.24f * disabledOpacity), switchX - 4.f, switchY - 5.f, 3.5f, &mBlend);
    g.DrawCircle(COLOR_BLACK.WithOpacity(0.75f * disabledOpacity), switchX, switchY, 14.5f, &mBlend, 1.2f);
    if (enabled)
      g.DrawCircle(IColor(255, 255, 174, 174).WithOpacity(disabledOpacity), switchX, switchY, 18.f, &mBlend, 1.2f);

    const auto stateArea = IRECT(mRECT.L, mRECT.T + 46.f, mRECT.R, mRECT.B);
...
{
public:
  NAMScaledBitmapControl(const IRECT& bounds, const IBitmap& bitmap)
  : IControl(bounds)
  , IBitmapBase(bitmap)
  {
    mIgnoreMouse = true;
...
public:
  ...
  void DrawLabel(IGraphics& g) override
  {
    ...
    {
      const IRECT labelBounds = mLabelBounds.GetTranslated(0.f, 6.f);
      g.DrawText(mStyle.labelText, mLabelStr.Get(), labelBounds, &mBlend);
    }
  }
  ...
  void DrawValue(IGraphics& g, bool mouseOver) override
  {
    ...
    if (mStyle.showValue)
    {
      const IRECT valueBounds = mValueBounds.GetTranslated(0.f, -12.f);
      g.DrawText(mStyle.valueText, mValueStr.Get(), valueBounds, &mBlend);
    }
  }
};
...
public:
  ...
  void DrawLabel(IGraphics& g) override
  {
    if (mLabelBounds.H() && mStyle.showLabel)
    {
      const IRECT labelBounds = mLabelBounds.GetTranslated(0.f, -4.f);
      g.DrawText(mStyle.labelText, mLabelStr.Get(), labelBounds, &mBlend);
    }
  }
};
...
public:
  ...
  : IVRadioButtonControl(
      bounds, paramIdx, {"Raw", "Normalized", "Calibrated"}, "Output Mode", style, EVShape::Ellipse,
      EDirection::Vertical, buttonSize) {};
  ...
  void SetNormalizedDisable(const bool disable)
  {
    ...
    mNormalizedDisabled = disable;
    SetStateDisabled(1, disable);
  };
  ...
  void OnMouseDown(float x, float y, const IMouseMod& mod) override
  {
    ...
    if ((index == 1 && mNormalizedDisabled) || (index == 2 && mCalibratedDisabled))
      return;
...
private:
  bool mNormalizedDisabled = false;
  bool mCalibratedDisabled = false;
};

class NAMSettingsPageControl : public IContainerBaseWithNamedChildren, public IBitmapBase
{
public:
  NAMSettingsPageControl(const IRECT& bounds, const IBitmap& bitmap, const IBitmap& inputLevelBackgroundBitmap,
                         const IBitmap& switchBitmap, const ISVG& closeSVG, const IVStyle& style,
  ...
  void ClearModelInfo()
  {
    ...
    auto* status = static_cast<IVLabelControl*>(GetNamedChild(mControlNames.inputCalibrationStatus));
    status->SetStr("");
    status->SetDirty(false);
    auto* heading = static_cast<IVLabelControl*>(GetNamedChild(mControlNames.inputCalibrationHeading));
    heading->SetStr("Input Calibration");
    heading->SetDirty(false);
  }
  ...
  void SetInputCalibrationAvailable(bool available)
  {
    ...
    status->SetDirty(false);
    auto* heading = static_cast<IVLabelControl*>(GetNamedChild(mControlNames.inputCalibrationHeading));
    heading->SetStr(available ? "Input Calibration" : "Input Calibration (not supported)");
    heading->SetDirty(false);
  }
  ...
  void OnAttached() override
  {
    ...
    const auto text = IText(16.f, EAlign::Center, PluginColors::HELP_TEXT);
    ...
    // Attach input/output calibration controls
    {
      ...
      const auto inputHeadingArea =
        IRECT(inputArea.L, inputLevelArea.T - 32.0f, inputArea.R, inputLevelArea.T - 6.0f);
      const auto inputStatusArea =
        IRECT(inputArea.L, inputSwitchArea.B + 1.0f, inputArea.R, inputSwitchArea.B + 20.0f);

      AddNamedChildControl(new IVLabelControl(
                             inputHeadingArea, "Input Calibration",
                             style.WithValueText(IText(18.f, EAlign::Center, PluginColors::HELP_TEXT))),
                           mControlNames.inputCalibrationHeading);
      ...
      AddNamedChildControl(new IVLabelControl(
                             inputStatusArea, "",
                             style.WithValueText(IText(15.f, EAlign::Center, PluginColors::HELP_TEXT))),
                           mControlNames.inputCalibrationStatus);
...
private:
  // IBitmap mBitmap; // Removed as we inherit from IBitmapBase now
  ...
  struct ControlNames
  {
    ...
    const std::string inputCalibrationHeading = "InputCalibrationHeading";
    const std::string inputCalibrationLevel = "InputCalibrationLevel";
    const std::string inputCalibrationStatus = "InputCalibrationStatus";
  ...
  public:
    ...
    void OnAttached() override
    {
      ...
      AddChildControl(new IVLabelControl(GetRECT().SubRectVertical(5, 0), "DESSMETAL", mStyle));
      AddChildControl(new IVLabelControl(GetRECT().SubRectVertical(5, 1), "By Alexander Dess", mStyle));
      AddChildControl(new IVLabelControl(GetRECT().SubRectVertical(5, 2), buildInfoStr.Get(), mStyle));
      AddChildControl(new IVLabelControl(GetRECT().SubRectVertical(5, 3), "Fork of Neural Amp Modeler", mStyle));
      AddChildControl(new IURLControl(GetRECT().SubRectVertical(5, 4), "Original by Steven Atkinson",
