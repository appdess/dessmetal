...
void dsp::tone_stack::BasicNamToneStack::Reset(const double sampleRate, const int maxBlockSize)
{
  ...
  mToneBass.PrepareBuffers(1, maxBlockSize);
  mToneMid.PrepareBuffers(1, maxBlockSize);
  mToneTreble.PrepareBuffers(1, maxBlockSize);
...
void dsp::tone_stack::BasicNamToneStack::SetParam(const std::string_view name, const double val)
{
  if (name == "bass")
  {
